...
Runs a full agent-first pipeline powered by the official OKX Agent Trade Kit:
...
如果配置文件不存在，不要立刻运行数据抓取流程，而是先进入引导流程。
...
- 更推荐由用户自行保存到 `~/.okx/config.toml`，再由官方 OKX Agent Trade Kit 读取
...
这个 skill 会通过官方 OKX Agent Trade Kit 读取你的历史交易数据，不会帮你下单，也不能提币。为了安全起见，请你创建一个只开启“读取”权限的 OKX API Key，不要开启“交易”或“提币”。

创建完成后，不用把 API Key 直接发给我。你只需要把它保存到 `~/.okx/config.toml`，我再继续帮你调用官方 OKX Agent Trade Kit 抓取数据、生成报告和网页。
```
...
1. Help the user set up `~/.okx/config.toml` for the official OKX Agent Trade Kit if it is missing
...
- 如果用户还没有 API Key，先引导他们去创建只读 API，供官方 OKX Agent Trade Kit 使用
- 如果用户已经有 API Key，不要要求他们把 key 直接发给你
- 优先引导用户把 key 保存进 `~/.okx/config.toml`，再由官方 OKX Agent Trade Kit 读取
...
- The pipeline clears `OKX_*` environment variables for the process so they cannot override `~/.okx/config.toml`, which is the credential source used by the official OKX Agent Trade Kit.
